...
# -----------------------------------------------------------------------------
# Alert simulation routes
# -----------------------------------------------------------------------------

ENABLE_ALERT_SIMULATION = as_bool(os.environ.get("ENABLE_ALERT_SIMULATION"), default=False)
ALERT_SIMULATION_TOKEN = (os.environ.get("ALERT_SIMULATION_TOKEN") or "").strip()


def _require_alert_simulation_access() -> None:
    if not ENABLE_ALERT_SIMULATION:
        abort(404)
    if ALERT_SIMULATION_TOKEN:
        provided_token = (request.headers.get("X-Alert-Sim-Token") or "").strip()
        if not secrets.compare_digest(provided_token, ALERT_SIMULATION_TOKEN):
            abort(403)


def _bounded_positive_int(raw_value: str | None, default_value: int, max_value: int) -> int:
    try:
        parsed = int(raw_value or str(default_value))
    except (TypeError, ValueError):
...
def simulate_error_500():
    ...
    app.logger.error(
        "simulated_500",
        extra={"event": "simulated_500", "path": request.path, "ip": get_client_ip()},
    )
    raise RuntimeError("Intentional 500 for alert simulation.")


@app.route("/simulate/login-failures", methods=["GET"])
def simulate_login_failures():
    _require_alert_simulation_access()
    count = _bounded_positive_int(request.args.get("count"), default_value=5, max_value=500)
    username = (request.args.get("username") or "sim-user").strip()[:80]
    for _ in range(count):
        app.logger.warning(
            "login_fail",
            extra={"event": "login_fail", "user": username, "ip": get_client_ip()},
        )
    return {"ok": True, "simulated_login_failures": count, "user": username}, 200


@app.route("/simulate/traffic-spike", methods=["GET"])
def simulate_traffic_spike():
    _require_alert_simulation_access()
    burst = _bounded_positive_int(request.args.get("burst"), default_value=100, max_value=5000)
    for i in range(burst):
        app.logger.warning(
            "simulated_traffic_spike",
            extra={
                "event": "simulated_traffic_spike",
                "path": request.path,
                "ip": get_client_ip(),
                "action": "synthetic_request",
                "comment_id": i + 1,
            },
        )
    return {"ok": True, "simulated_traffic_events": burst}, 200
...
def login():
    ...
    if form.validate_on_submit():
        ...
        is_blocked, retry_after = _rate_limit_login_failures(client_ip)
        if is_blocked:
            app.logger.warning(
                "login_rate_limited",
                extra={
            ...
            flash(f"Demasiados intentos. Intenta en {retry_after}s.", "error")
        ...
        if user and check_password_hash(user.password_hash, password):
            ...
            app.logger.info(
                "login_ok",
                extra={"event": "login_ok", "user": username, "ip": client_ip},
            )
            flash("Login OK.", "ok")
        ...
        app.logger.warning(
            "login_fail",
            extra={"event": "login_fail", "user": username, "ip": client_ip},
        )
